...
describe('FreeLayoutPanel', () => {
  ...
  it('affiche les boutons de navigation du rail', () => {
    render(<FreeLayoutPanel />);
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabCanvas)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabMatch)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabText)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabData)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabPlayers)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabMedia)).toBeInTheDocument();
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabComposed)).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(CUSTOM_FIELD_LABELS.freeLayoutTabProperties)).toBeInTheDocument();
  });
  ...
  it('affiche le message de s\u00e9lection vide dans l\'onglet propri\u00e9t\u00e9s', () => {
    useEditorUIStore.setState({ activeFreeLayoutTab: 'properties' });
    render(<FreeLayoutPanel />);
    expect(screen.getByText(CUSTOM_FIELD_LABELS.freeLayoutNoSelection)).toBeInTheDocument();
  });

  it('affiche les propri\u00e9t\u00e9s d\'un champ s\u00e9lectionn\u00e9', () => {
    useEditorUIStore.setState({ activeFreeLayoutTab: 'properties' });
    const element = {
